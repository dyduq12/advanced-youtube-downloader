...
echo %MSG_LINK%
:: Usa PowerShell para criar o atalho com seguranca (evita bugs de espacos no VBS/Reg)
powershell -NoProfile -Command "$w = New-Object -ComObject WScript.Shell; $s = $w.CreateShortcut([Environment]::GetFolderPath('Desktop') + '\%ATALHO_NOME%.lnk'); $s.TargetPath = 'powershell.exe'; $s.Arguments = '-ExecutionPolicy Bypass -WindowStyle Hidden -File \"%PASTA_DESTINO%\%ARQUIVO_SCRIPT%\"'; $s.WorkingDirectory = '%PASTA_DESTINO%'; if (Test-Path '%PASTA_DESTINO%\Icon.ico') { $s.IconLocation = '%PASTA_DESTINO%\Icon.ico' }; $s.Save()"
